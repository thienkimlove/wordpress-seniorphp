...
function clonetemplates_paging_nav() {
	...
	<nav class="navigation paging-navigation" role="navigation">
		...
		<div class="nav-links">
			...
			<div class="nav-previous"><?php next_posts_link( __( '<span class="meta-nav">&larr;</span> Older posts', 'magcrunch' ) ); ?></div>
			<?php endif; ?>

			<?php if ( get_previous_posts_link() ) : ?>
			<div class="nav-next"><?php previous_posts_link( __( 'Newer posts <span class="meta-nav">&rarr;</span>', 'magcrunch' ) ); ?></div>
			<?php endif; ?>
...
function clonetemplates_post_nav() {
	...
	<nav class="navigation post-navigation" role="navigation">
		<h1 class="screen-reader-text"><?php _e( 'Post navigation', 'magcrunch' ); ?></h1>
		<div class="nav-links">
			<?php
				previous_post_link( '<div class="nav-previous">%link</div>', _x( '<span class="meta-nav">&larr;</span>&nbsp;%title', 'Previous post link', 'magcrunch' ) );
				next_post_link(     '<div class="nav-next">%link</div>',     _x( '%title&nbsp;<span class="meta-nav">&rarr;</span>', 'Next post link',     'magcrunch' ) );
			?>
		</div><!-- .nav-links -->
	</nav><!-- .navigation -->
	<?php
...
function clonetemplates_posted_on() {
	...
	$posted_on = sprintf(
		_x( 'Posted on %s', 'post date', 'magcrunch' ),
		'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
	);

	$byline = sprintf(
		_x( 'by %s', 'post author', 'magcrunch' ),
...
function clonetemplates_wp_title( $title, $sep ) {
	...
	if ( ( $paged >= 2 || $page >= 2 ) && ! is_404() ) {
		$title .= " $sep " . sprintf( __( 'Page %s', 'magcrunch' ), max( $paged, $page ) );
	}
